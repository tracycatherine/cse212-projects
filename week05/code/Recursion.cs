...
public static class Recursion
{
    ...
    {
        // Base case: if word is the desired size, add to results
        if (word.Length == size)
        {
            results.Add(word);
            return;
        ...
        {
            // Skip letters already used in word
    ...
    public static void SolveMaze(List<string> results, Maze maze, int x = 0, int y = 0, List<ValueTuple<int, int>>? currPath = null)
    {
        ...
        // No need to remove last position due to new List for each branch
    }
}